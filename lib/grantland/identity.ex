defmodule Grantland.Identity do
  @moduledoc """
  ...
  def create_user(attrs \\ %{}) do
    case %User{}
         ...
         |> Repo.insert() do
      {:ok, user} -> broadcast({:ok, user}, :user_created)
  ...
  def update_user(%User{} = user, attrs) do
    case user
         |> User.registration_changeset(attrs)
         |> Repo.update() do
      {:ok, user} -> broadcast({:ok, user}, :user_updated)
  ...
  def roles_for_view do
    Enum.map(Ecto.Enum.values(User, :role), &{Phoenix.Naming.humanize(&1), &1})
  end

  def subscribe do
    Phoenix.PubSub.subscribe(Grantland.PubSub, "users")
  end

  defp broadcast({:error, _reason} = error, _event), do: error

  defp broadcast({:ok, user}, event) do
    Phoenix.PubSub.broadcast(Grantland.PubSub, "users", {event, user})
    {:ok, user}
  end

  alias Grantland.Identity.{User, UserToken, UserNotifier}
